...
class EMPCustomerVisitController extends Controller
{
    public function index(Request $request,$id)
    {
        ...
        try
        {
            ...
            return view('employee.new_visite.index', compact('Category','Customer','id','Followups','Products','CustProducts','employees','feedback','closereason','color','branches','vendor','purity','orderStatus'));
       } catch (\Exception $e) 
        {
            report($e);
            return false;
    ...
    public function today()
{
  $emp_id = Auth::user()->emp_id;
    ...
    public function overdue()
    {
        ...
        return view('employee.cust_followup.overdue_followup', compact('followups'));
    }
    ...
    public function create($id)
    {
        ...
        $CustProducts = CustomerProduct::with('category','product','employee','orderDetails.OrderStatus')->where(['cust_id'=>$id])->get();
        $Products = Product::orderBy('product_id', 'desc')->get();
    ...
    public function view_visit(Request $request,$id)
    {
        try
        {
            ...
            $Followups = CustomerVisit::with('visitDetails','closereason')->where(['visit_id'=>$id])->get();
            $CustProducts = CustomerProduct::with('category','product','employee','orderDetails.OrderStatus')->where(['visit_id'=>$id])->get();
            $Products = Product::orderBy('product_id', 'desc')->get();
            $employees = Employee::where(['iStatus'=>1,'isDelete'=>0,'role_id'=>2])->orderBy('emp_name', 'asc')->get();
            $feedback = CustomerVisit::with('visitDetails','closereason')->where(['visit_id'=>$id])->latest()->first();
            ...
            $lastVisite = CustomerVisit::orderBy('visit_id', 'desc')->first();
                
                    // Extract the numeric part and increment it
                    if ($lastVisite) 
                    {
                        $lastNumber = (int)substr($lastVisite->visit_id, 1); // Get the number part
                        $nextNumber = $lastNumber + 1;
                    } else {
                        $nextNumber = 1; // Start from L001 if no records exist
                    }
                
                    // Format the next job card number
                    $newVisiteNo = 'v' . str_pad($nextNumber, 3, '0', STR_PAD_LEFT); // E.g., L002

            return view('employee.new_visite.view_visit', compact('Category','Customer','id','Followups','Products','CustProducts','employees','feedback','closereason','newVisiteNo'));
       } catch (\Exception $e) 
        {
            report($e);
            return false;
